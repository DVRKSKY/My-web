...
export default function CardProyects({ title, text, url, image }) {
  ...
  const imageClassNames = isLoaded ? 'h-[50%] w-[100%] rounded-t-lg object-cover loaded' : 'h-[50%] w-[100%] rounded-t-lg object-cover';
  ...
  return (
    <div className='md:w-[30%] w-full h-[440px] ma-auto m-5 rounded-lg justify-center flex flex-col items-center border-tertiary-100 border hover:shadow-lg'>
      ...
      <div className='w-full h-[50%] flex flex-col justify-center items-start md:px-6 px-4 '>
        ...
        {url === "" ? (
          <span className='my-2 text-tertiary cursor-disabled'>Just designed</span>
        ):(
          <a href={`http://${url}`} target="_blank" rel="noreferrer" className='my-2 text-primary cursor-pointer '>Visit proyect</a>
        )}
